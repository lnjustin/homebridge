...
def deviceSelectPage() {
    return dynamicPage(name: 'deviceSelectPage', title: sBLANK, install: false, uninstall: false) {
        section(sectHead('Define Specific Categories:')) {
            ...
            input 'securityKeypadsList', 'capability.securityKeypad', title: inTS1("Security Keypads: (${securityKeypadsList ? securityKeypadsList.size() : 0} Selected)", 'security_keypad'), description: inputFooter(sTTS, sCLRGRY, true), multiple: true, submitOnChange: true, required: false
        }
    ...
    if (curType && obj) {
        if (curType == 'Security Keypad') {
            return [
                ...
                commands: [],
                attributes: ['alarmSystemStatus': sItem?.currentValue('securityKeypad')?.toString()]
            ]            
        }
        else {
            return [
